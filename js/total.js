...
$('.buttons .btn2').on('click', function () {
    ...
    $.getJSON('users.json', function (data) {
        $.each(data, function (i, val) {
            var htm = '<div class="user">' + '<div class="name">' + val.name + '</div>' + '<div class="username">' + val.username + '</div>' + '<div class="email">' + val.email + '</div>' + '<div class="adress">' + '<div class="street">' + val.address.street + '</div>' + '<div class="suite">' + val.address.suite + '</div>' + '<div class="city">' + val.address.city + '</div>' + '</div>' + '<div class="phone">' + val.phone + '</div>' + '</div>';
...
//text from document
function getText() {
    fetch('js/simple.txt').then(function (res) {
        return res.text();
    }).then(function (data) {
...
var fadeInGallery = {
    ...
    changeFoto: function changeFoto(e) {
        var me = fadeInGallery;
        me.imgs.forEach(function (img) {
            img.style.opacity = 1;
        });
...
function geoCheck() {
    ...
    var geoBtn = document.querySelector('.geoBtn');
...
function myCss() {
    NodeList.prototype.css = function (propertyOrObject, value) {
        Array.prototype.forEach.call(this, function (elem) {
            if ((typeof propertyOrObject === 'undefined' ? 'undefined' : _typeof(propertyOrObject)) === "object") {
...
var map = {
    init: function init() {
        ...
        this.path = new google.maps.DirectionsService(), this.pathRender = new google.maps.DirectionsRenderer();
    ...
    makeMap: function makeMap() {
        ...
        var marker = new google.maps.Marker({
            ...
            icon: "src/img/map_marker.png"
        });
    },
    //funkcja przygotowująca trase dojazdu doc celu
    prepareRoute: function prepareRoute(coords) {
        var renderOptions = {
    ...
    handleRoute: function handleRoute(result, status) {
    ...
    checkGeoSupport: function checkGeoSupport() {
        if (navigator.geolocation) {
    ...
    getGeoData: function getGeoData() {
        ...
        var errorResult = function errorResult(errorObj) {
...
function VideoPlayer(url, place) {
    ...
    if (!document.createElement("video").canPlayType) {
        //sprawdzenie czy utworzone video jest obsługiwane przez przeglądarkę  
...
VideoPlayer.prototype.updatePlayingProgress = function () {
    var procentPlayed = this.video.currentTime / this.video.duration * 100;
...
VideoPlayer.prototype.formatTime = function (seconds) {
    ...
    if (remainingSeconds == 0) remainingSeconds = "00";else if (remainingSeconds < 10) remainingSeconds = "0" + remainingSeconds;
...
VideoPlayer.prototype.setCurrentPlayback = function (e) {
    ...
    var newTime = this.video.duration * percent; //310*0.5=155s
...
var myOwnVideoPlayer = $('#myOwnVideoPlayer');

var myUrl = 'src/media/ollie.mp4#t=3';
var myUrl2 = 'src/media/ollie.mp4#t=12';
var videoPlayer1 = new VideoPlayer(myUrl, myOwnVideoPlayer);
var videoPlayer2 = new VideoPlayer(myUrl2, myOwnVideoPlayer);

var url3 = 'src/media/ollie.mp4';
var place3 = $('.showRoad');
