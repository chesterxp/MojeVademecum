...
var processPayment = function processPayment(responseArray) {
    var order = responseArray[0]; //tablica z zamówieniem
    var total = responseArray[1]; //pełna kwota zamówienia
    return new Promise(function (resolve, reject) {
        setTimeout(function () {
...
//SET
var tab = [1, 2, 3, 4, 56, 2, 3, 4, 3, 2, 1, 1, 2, 3, 4, 5, 6];
console.log('tab.length: ', tab.length);
//Przykład
var tab2 = new Set(tab);
tab2.add(31);
...
//Map
var komputer = [{ name: 'pierwszy komp', rok: 2010, number: 1 }, { name: 'drugi komp', rok: 2013, number: 2 }, { name: 'trzeci komp', rok: 2016, number: 3 }];
var oprogramowanie = [{ system: 'Windows', oprogramowanie: 'legalne', number: 1 }, { system: 'Linux', oprogramowanie: 'legalne', number: 2 }, { system: 'IOS', oprogramowanie: 'legalne', number: 3 }];
var mapa = new Map();
...
var Order = function () {
    ...
    _createClass(Order, [{
        ...
        key: 'addDish',
        value: function addDish(dish) {
            this.dishes.add(dish);
        }
    }], [{
        key: 'getErrors',
        value: function getErrors(dish) {
            var errors = {};
            if (!dish.name) {
...
var orders = [new Order(1, [{ name: 'Steak', quantity: 1, details: 'Well done' }, { name: 'Fish', quantity: 1 }]), new Order(2, [{ name: 'Halloumi salad', quantity: 1 }, { name: 'Greek salad', quantity: 1 }, { name: 'Espresso', quantity: 2 }])];
var tables = [{
    tableName: 'Table 1',
    capacity: 4,
    orderId: 1
...
var tablesToOrders = new Map();

tables.forEach(function (table) {
    tablesToOrders.set(table, orders.find(function (o) {
        return o.id === table.orderId;
    }));
});
...
//text from JSON
function getJson() {
    fetch('js/users.json').then(function (res) {
        return res.json();
    }).then(function (data) {
        var output = '';
        data.forEach(function (user) {
            output += '<div class="person">' + '<div class="firstName">' + user.name + '</div>' + '<div class="lastName">' + user.username + '</div>' + '<div class="email">' + user.email + '</div>' + '</div>';
        });
